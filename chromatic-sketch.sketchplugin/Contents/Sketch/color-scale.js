...
exports['default'] = function (context) {
  ...
  selection.iterate(function (layer) {
    if (layer.isShape) {
      layerColors.push(String(layer.style.sketchObject.fills().firstObject().color().immutableModelObject().stringValueWithAlpha(true)));
    }
  });
  ...
  var handlers = {
    ready: function () {
      function ready() {
        webview.eval('window.renderColorScaleView(\'' + String(layerColors[0] || '#dddddd') + '\', \'' + String(layerColors[1] || '#000000') + '\')');
      }

      return ready;
    }(),
    insert: function () {
      function insert(colorArray) {
        webview.close();
        var group = page.newGroup({
          frame: new sketch.Rectangle(0, 0, PALETTE_WIDTH, PALETTE_HEIGHT),
          name: 'Color Scale'
        });
        var swatchWidth = PALETTE_WIDTH / colorArray.length;

        for (var i = 0; i < colorArray.length; i++) {
          var myStyle = new sketch.Style();
          myStyle.borders = '';
          myStyle.sketchObject.fills().firstObject().setColor((0, _helpers.makeColor)(colorArray[i]));
  ...
  var webview = (0, _helpers.createWebview)(context, handlers, 'Create Color Scale', 435);
};

var _helpers = __webpack_require__(1);

var PALETTE_WIDTH = 550;
